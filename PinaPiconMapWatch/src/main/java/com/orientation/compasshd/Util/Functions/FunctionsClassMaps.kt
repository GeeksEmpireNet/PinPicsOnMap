...
class FunctionsClassMaps {
    ...
    fun locationDetail(latLng: LatLng): String {
        var locationInfo: String = "Unknown (${latLng.latitude}.${latLng.longitude} "
        try {
            val geocoder: Geocoder = Geocoder(context, Locale.getDefault())
            ...
            if (addresses.size > 0) {
                ...
                if (knownName != null) {
                    locationInfo = "$country, $state, $city, $knownName"
                } else {
                    locationInfo = "$country, $state, $city"
                }
            } else {
            }
        } catch (e: Exception) {
